...
import { getKatagoUrl } from './katagoConfig.js';
...
export function getAnalysisQueue(options = { startWorker: false, concurrency: 1 }) {
    ...
    */
    ...
    // 返回一个 Mock 对象避免报错
    return {
        add: async () => ({ id: 'mock-job-' + Date.now(), queue: {} }),
        getJob: async (jobId) => {
            if (jobId && jobId.toString().startsWith('mock-job-')) {
                return {
                    id: jobId,
                    getState: async () => 'failed', // Mock jobs immediately fail since there is no worker
                    progress: () => 0,
                    failedReason: "Redis queue is disabled in this environment."
                };
            }
            return null;
        },
...
async function processAnalysisJob(job) {
    ...
    try {
        ...
        // 动态获取最合适的 KataGo 服务地址 (优先使用 Nginx LB)
        const currentKatagoUrl = await getKatagoUrl();
        ...
        for (let i = 0; i < totalSteps; i++) {
            ...
            // 调用 KataGo
            const response = await fetch(`${currentKatagoUrl}/analyze`, {
                method: 'POST',
